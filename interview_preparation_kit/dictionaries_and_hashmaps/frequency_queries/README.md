...
* 则需要设置两个`unordered_map`, 变量名分别为`integers`和`nums_of_integers`, 并分别用于满足上面两点需求. integers把queries[i][1]作为key, key的计数作为value, 而nums_of_integers把integers.second作为key, key的计数作为value.
